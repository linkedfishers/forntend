...
const equipmentIconUrl = 'assets/leaflet/post-icon.png';
...
export class MapComponent implements AfterViewInit {
    ...
    this.equipmentService.getHebergements().subscribe(
      (res) => {
        ...
        this.homes.forEach((hebergement) => {
          ...
          this.homesMarker.push(
            L.marker(hebergement.position.coordinates, {
              icon: homeIcon,
              draggable: false,
              autoPan: true,
            })
    ...
    this.equipmentService.getBoats().subscribe(
      (res) => {
        ...
        this.boat.forEach((boat) => {
          ...
          this.boatMarker.push(
            L.marker(boat.position.coordinates, {
              icon: boatIcon,
              draggable: false,
              autoPan: true,
            }).bindPopup(
              `<a target="_blank" href="/details-boat/${boat?._id}"><h1>${
                boat?.name
              }</h1><a><h4>${boat?.type?.name || ''}</h4>`
            )
          );
    ...
    this.equipmentService.getEquipments().subscribe(
      (res) => {
        ...
        this.equipments.forEach((equipment) => {
          ...
          this.equipmenetMarker.push(
            L.marker(equipment.position.coordinates, {
              icon: equipmenetIcon,
              draggable: false,
              autoPan: true,
            })
              .bindPopup(`<a target="_blank" href="/details-home/${equipment?._id}" ><h1>${equipment.name}</h1><a> by <a target="_blank" href="/profile/${equipment?.owner?.slug}">
    ...
    this.eventService.getAll().subscribe(
      (res) => {
        ...
        this.events.forEach((event) => {
          ...
          this.eventsMarker.push(
            L.marker(event.position.coordinates, {
              icon: eventIcon,
              draggable: false,
              autoPan: true,
            }).bindPopup(
              `<a target="_blank" href="/details-event/${event?._id}"><h1>${event.name}</h1><a>`
            )
          );
  ...
  updateCircle() {
    ...
    if (this.showPosts) {
      this.postsMarker.forEach((marker) => {
    ...
    if (this.showEvents) {
      this.eventsMarker.forEach((marker) => {
        let distance = this.getDistanceFromLatLonInKm(
          this.currentPositionMarker,
          marker
        );
